...
if ($is_preview && isset($block['data']['preview_image_help'])) {
    $preview_image_url = get_template_directory_uri() . '/blocks/values/' . $block['data']['preview_image_help'];
...
// Block classes
$class_name = 'values-block';
if (!empty($block['className'])) {
    $class_name .= ' ' . $block['className'];
}

// Fields
$title = get_field('title') ?: 'ЦІННОСТІ ПРОЄКТУ';
$values_list = get_field('values_list');
$btn_text = get_field('button_text') ?: 'ПЕРЕЙТИ ДО ПРОДУКТІВ';
...
<section <?= $anchor ?> class="<?= esc_attr($class_name) ?>">
    <div class="container">
        <div class="values__inner">
            <?php if ($title): ?>
                <h2 class="values__title reveal visible">
                    <?= esc_html($title); ?>
                </h2>
            <?php endif; ?>
                ...
                <div class="values__grid">
                    <?php foreach ($values_list as $index => $item): ?>
                        <div class="values__card">
                            ...
                            <div class="values__card-content">
                                <h3 class="values__card-title"><?= esc_html($item['title']); ?></h3>
                                <p class="values__card-desc"><?= esc_html($item['description']); ?></p>
                            </div>
                        </div>
                    <?php endforeach; ?>
                </div>
            ...
            <div class="values__footer reveal visible">
